...
#define DEF_FREQUENCY_DOWN_THRESHOLD_SUSPENDED	(35)
#define DEF_FREQUENCY_STEP			(15)
#define DEF_SAMPLING_RATE			(40000)
#define DEF_IGNORE_NICE_LOAD			(0) /* added */
#define DEF_BOOST_ENABLED			(0)
#define DEF_BOOST_COUNT				(0)
...
/*	if (!display_on && policy->cur == policy->min) */
	if (state_suspended && policy->cur == policy->min) /* added */
		return;
...
static struct cs_dbs_tuners *alloc_tuners(struct cpufreq_policy *policy)
{
	...
	tuners->ignore_nice_load = DEF_IGNORE_NICE_LOAD; /* added */
