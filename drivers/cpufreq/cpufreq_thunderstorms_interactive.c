/*
 * drivers/cpufreq/cpufreq_thunderstorms_interactive.c
 *
...
#include <trace/events/cpufreq_thunderstorms_interactive.h>
...
struct cpufreq_thunderstorms_interactive_cpuinfo {
...
static DEFINE_PER_CPU(struct cpufreq_thunderstorms_interactive_cpuinfo, cpuinfo);
...
static int cpufreq_governor_thunderstorms_interactive(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_THUNDERSTORMS_INTERACTIVE
static
#endif
struct cpufreq_governor cpufreq_gov_thunderstorms_interactive = {
	.name = "ts_interactive",
	.governor = cpufreq_governor_thunderstorms_interactive,
	.max_transition_latency = 10000000,
	.owner = THIS_MODULE,
};

static void cpufreq_thunderstorms_interactive_timer(unsigned long data)
{
	...
	u64 time_in_idle;
	u64 idle_exit_time;
	struct cpufreq_thunderstorms_interactive_cpuinfo *pcpu =
	...
	if (cpu_load >= go_hispeed_load || boost_val) {
		if (pcpu->target_freq <= pcpu->policy->min) {
			...
			if (pcpu->target_freq == hispeed_freq &&
				...
				trace_cpufreq_thunderstorms_interactive_notyet(data, cpu_load,
								 pcpu->target_freq,
								 new_freq);
				goto rearm;
			}
	...
	if (pcpu->target_freq == new_freq) {
		trace_cpufreq_thunderstorms_interactive_already(data, cpu_load,
						  pcpu->target_freq, new_freq);
		goto rearm_if_notmax;
	}

	trace_cpufreq_thunderstorms_interactive_target(data, cpu_load, pcpu->target_freq,
					 new_freq);
...
static void cpufreq_thunderstorms_interactive_tune(struct work_struct *work)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	for_each_cpu(cpu, &tmp_mask) {
		...
		for_each_cpu(j, pcpu->policy->cpus) {
			struct cpufreq_thunderstorms_interactive_cpuinfo *pjcpu =
					&per_cpu(cpuinfo, j);
...
static void cpufreq_thunderstorms_interactive_idle_start(void)
{
	struct cpufreq_thunderstorms_interactive_cpuinfo *pcpu =
...
static void cpufreq_thunderstorms_interactive_idle_end(void)
{
	struct cpufreq_thunderstorms_interactive_cpuinfo *pcpu =
		&per_cpu(cpuinfo, smp_processor_id());
...
static int cpufreq_thunderstorms_interactive_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			if (max_freq != pcpu->policy->cur)
				__cpufreq_driver_target(pcpu->policy,
							max_freq,
							CPUFREQ_RELATION_H);
			trace_cpufreq_thunderstorms_interactive_setspeed(cpu,
                             pcpu->target_freq,
						     pcpu->policy->cur);
		}
	}
...
static void cpufreq_thunderstorms_interactive_boost(void)
{
	int i;
	int anyboost = 0;
	unsigned long flags;
			   ...
			   const char *buf, size_t count)
{
	...
	if (boost_val) {
		trace_cpufreq_thunderstorms_interactive_boost("on");
		cpufreq_thunderstorms_interactive_boost();
	} else {
		trace_cpufreq_thunderstorms_interactive_unboost("off");
	}
				...
				const char *buf, size_t count)
{
	...
	trace_cpufreq_thunderstorms_interactive_boost("pulse");
	cpufreq_thunderstorms_interactive_boost();
	return count;
}
...
static struct attribute *thunderstorms_interactive_attributes[] = {
...
static struct attribute_group thunderstorms_interactive_attr_group = {
	.attrs = thunderstorms_interactive_attributes,
	.name = "ts_interactive",
};

static int cpufreq_thunderstorms_interactive_idle_notifier(struct notifier_block *nb,
                                             unsigned long val,
                                             void *data)
{
	switch (val) {
        case IDLE_START:
            cpufreq_thunderstorms_interactive_idle_start();
            break;
        case IDLE_END:
            cpufreq_thunderstorms_interactive_idle_end();
            break;
	}
...
static struct notifier_block cpufreq_thunderstorms_interactive_idle_nb = {
	.notifier_call = cpufreq_thunderstorms_interactive_idle_notifier,
};
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		rc = sysfs_create_group(cpufreq_global_kobject,
				&thunderstorms_interactive_attr_group);
		if (rc)
			return rc;
            
        idle_notifier_register(&cpufreq_thunderstorms_interactive_idle_nb);
		break;
	...
	case CPUFREQ_GOV_STOP:
        ...
        idle_notifier_unregister(&cpufreq_thunderstorms_interactive_idle_nb);
...
static int __init cpufreq_thunderstorms_interactive_init(void)
{
	unsigned int i;
	struct cpufreq_thunderstorms_interactive_cpuinfo *pcpu;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_thunderstorms_interactive_timer;
    ...
    speedchange_task =
        kthread_create(cpufreq_thunderstorms_interactive_speedchange_task, NULL,
                "cfthunderstorms_interactive");
	...
	INIT_WORK(&tune_work,
		  cpufreq_thunderstorms_interactive_tune);
    
    /* NB: wake up so the thread does not look hung to the freezer */
    wake_up_process(speedchange_task);

	return cpufreq_register_governor(&cpufreq_gov_thunderstorms_interactive);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_thunderstorms_interactive
fs_initcall(cpufreq_thunderstorms_interactive_init);
#else
module_init(cpufreq_thunderstorms_interactive_init);
#endif

static void __exit cpufreq_thunderstorms_interactive_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_thunderstorms_interactive);
	kthread_stop(speedchange_task);
	put_task_struct(speedchange_task);
	destroy_workqueue(tune_wq);
}

module_exit(cpufreq_thunderstorms_interactive_exit);
